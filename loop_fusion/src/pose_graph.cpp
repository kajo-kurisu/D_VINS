...
float loop_top_thres;
float loop_back_thres;
...
//    }
    ...
    {
        loop_result = compressed_image.clone();
        if (!keyframe->top_sim_index.empty())
            putText(loop_result, "neighbour score:" + to_string(keyframe->top_sim[0]), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
    }

    // visual loop result
    if (DEBUG_IMAGE && frame_index > 5 && keyframe->top_sim_index.back()!=-1)
    {
        for (int i = 0; i < keyframe->top_sim_index.size(); i++)
        {
            int tmp_index = keyframe->top_sim_index[i];
            auto it = image_pool.find(tmp_index);
            cv::Mat tmp_image = (it->second).clone();

            putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(keyframe->top_sim[i]), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
            cv::hconcat(loop_result, tmp_image, loop_result);
        }
    }
    //原本的
...
//            {
////                ROS_INFO("THE SCORE OF RET = %lf",ret[i].Score);
////
////                cout<<ret[i].Score<<endl;
//                find_loop = true;
//                int tmp_index = ret[i].Id;
//                if (DEBUG_IMAGE && 0)
//                {
...
//        return -1;
    ...
    //mix_des
    //Ip
    if(!keyframe->top_sim_index.empty() && keyframe->top_sim[0]>loop_top_thres)
    //L2
//    if(keyframe->top_sim_index.size()>=1 && keyframe->top_sim[0]<0.007)
    {
...
//            }
    ...
    {
        int min_index = -1;
        for (unsigned int i = 0; i < keyframe->top_sim_index.size(); i++)
        {
            //IP
            if (min_index == -1 || (keyframe->top_sim_index[i]< min_index && keyframe->top_sim[i] > loop_back_thres))
            //L2
//            if (min_index == -1 || (keyframe->top_sim_index[i]< min_index && keyframe->top_sim[i] < 0.008))
                min_index = keyframe->top_sim_index[i];
        }
//        cv::Mat loop_show;
//        auto it = image_pool.find(min_index);
//        cv::Mat tmp_image = (it->second).clone();
//
////        auto now = image_pool.find(frame_index);
////        cv::Mat now_image = (it->second).clone();
//
//        auto loop_kf = getKeyFrame(min_index);
//        for(int i = 0; i< (int) loop_kf->keypoints.size(); i++)
//        {
//            //376*240
//            cv::Point2f cur_pt;
//            cur_pt.x = loop_kf->keypoints[i].pt.x * tmp_image.cols/width_adj;
//            cur_pt.y = loop_kf->keypoints[i].pt.y *  tmp_image.rows/height_adj;
//            cv::circle(tmp_image, cur_pt, 3, cv::Scalar(0, 255, 0));
//        }
//        putText(tmp_image, "index:  " + to_string(min_index) + "loop score:" , cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
//        cv::hconcat(loop_result, tmp_image, loop_result);
...
//        min_index = keyframe->top_sim_index[0];
        return min_index;
...
//        std::ofstream brief_file(brief_path, std::ios::binary);
        auto mix_path = POSE_GRAPH_SAVE_PATH + to_string((*it)->index) + "mix_des.dat";
        auto sp_des_path = POSE_GRAPH_SAVE_PATH + to_string((*it)->index) + "sp_des.dat";

        std::ofstream mix_file(mix_path);
        std::ofstream sp_file(sp_des_path);
        ...
        for(int i=0; i <512;++i)
        {
            mix_file << (*it)->mix_descriptors[i]<<" "<< endl;
        }
        ...
        {
            //sp_kpts  -> point2d
            for(auto &des:(*it)->local_descriptors) sp_file<<des<<endl;
            //keypoint中已经包含sp_kpts了，且顺序为sp -> vins
...
void PoseGraph::loadPoseGraph()
{
                                    ...
                                    &keypoints_num) != EOF) 
    {
        ...
        */
        ...
        auto mix_path = POSE_GRAPH_SAVE_PATH + to_string(index) + "mix_des.dat";
        auto sp_des_path = POSE_GRAPH_SAVE_PATH + to_string(index) + "sp_des.dat";

        FILE *mix_file;
        mix_file = fopen(mix_path.c_str(), "r");

        FILE *sp_des_file;
        sp_des_file = fopen(sp_des_path.c_str(), "r");
...
//        std::ifstream brief_file(brief_path, std::ios::binary);
        ...
        //mix_des dim = 512
        std::vector<float> mix_descriptors;
        std::vector<int> top_sim_index;
        std::vector<float> top_sim;


        //local des  先sp再point2d
...
//            brief_descriptors.push_back(tmp_des);
            ...
            for(int j = 0;j<256;++j)
            {
                fscanf(sp_des_file,"%f ", &local_descriptors[j]);
            }
