...
static auto mix = MixVPR_net::creat_mix(mix_engine_path, 0);
...
void KeyFrame::compute_mix_des_test() {
//    static auto mix = MixVPR_net::creat_mix(mix_engine_path,0);
    mix->mix_des.clear();
    mix->mix_extractor(image);
    mix_descriptors = mix->mix_des;
    descriptors_database.insert(descriptors_database.end(),mix_descriptors.data(),mix_descriptors.data() + 512);
}
...
void KeyFrame::computeSuperpoint(){
    ...
    if(USE_SP)
    {
        ...
        for(auto & i : _keypoints)
        {
            ...
            sp_keypoints.push_back(key);
        }
    }
    else local_descriptors.clear();
...
//    cout<<image.rows<<" "<<image.cols<<endl;
    deep_net_estimator_lg->lg_matcher(_keypoints, _keypoints_old, window_local_descriptors, local_descriptors_old,image.rows,image.cols,image.rows,image.cols);
//    deep_net_estimator_lg->lg_matcher(_keypoints, _keypoints_old, local_descriptors, local_descriptors_old,image.rows,image.cols,image.rows,image.cols);
...
//        cout<<matched_3d[i]<<matched_2d_old_norm[i]<<endl;
    ...
    solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 200, PNP_INFLATION/460.0, 0.99, inliers);
...
//    searchByBRIEFDes(matched_2d_old, matched_2d_old_norm, status, old_kf->brief_descriptors, old_kf->keypoints, old_kf->keypoints_norm);
    ...
    */
    ...
    cv::Mat old_image = old_kf->image.clone();
    cv::Mat now_image = image.clone();
    bool DEBUG = false;
    if(DEBUG)
    {
        cv::cvtColor(now_image,now_image,CV_GRAY2RGB);
        cv::cvtColor(old_image,old_image,CV_GRAY2RGB);
        for(int i = 0; i< (int) old_kf->keypoints.size(); i++)
        {
            //376*240
            cv::Point2f cur_pt;
            cur_pt = old_kf->keypoints[i].pt ;
            cv::circle(old_image, cur_pt, 5, cv::Scalar(0, 255, 0));
        }

        for(auto& keypoint : window_keypoints)
        {
            //376*240
            cv::Point2f cur_pt;
            cur_pt = keypoint.pt;
            cv::circle(now_image, cur_pt, 5, cv::Scalar(0, 255, 0));
        }

        putText(old_image, "old_index:  " + to_string(old_kf->index) + " kpts_num: " + to_string(old_kf->keypoints.size()), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,0,255),2);
        putText(now_image, "now_index:  " + to_string(index)+" kpts_num: " + to_string(window_keypoints.size()), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,0,255),2);
//            putText(now_image, "index:  " + to_string(index)+"kpts_num: " + to_string(keypoints.size()), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 2, cv::Scalar(0));

        cv::Mat test(image.rows,10,CV_8UC3,cv::Scalar(255,255,255));

        cv::hconcat(now_image, test, now_image);
        cv::hconcat(now_image, old_image, now_image);

        if(matched_2d_cur.size() == matched_2d_old.size())
        {
            for (int i = 0; i< (int)matched_2d_cur.size(); i++)
            {
                cv::Point2f old_pt = matched_2d_old[i];
                old_pt.x += (image.cols + 10) ;
                cv::line(now_image, matched_2d_cur[i], old_pt, cv::Scalar(0, 255, 0), 1, 8, 0);
            }
        }

        cv::imshow("debug",now_image);
        cv::waitKey(5);
    }
