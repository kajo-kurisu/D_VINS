...
namespace TRT {
    ...
    public:
        ...
        Tensor& set_kpts_to_tensor(const std::vector<cv::Point2f>& kpts);
        Tensor &set_vec_to_tensor(const std::vector<float> &vec);
        ...
        Tensor &set_norm_mat_gray(int n, const cv::Mat &image);
