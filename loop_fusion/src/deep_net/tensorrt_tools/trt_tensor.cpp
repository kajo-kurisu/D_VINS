...
namespace TRT{
    ...
    Tensor &Tensor::set_kpts_to_tensor(const vector<cv::Point2f> &kpts)  {
        to_cpu(false);
        std::memcpy(cpu<float>(),kpts.data(),kpts.size()*2*sizeof(float) );
        return *this;
    }

    Tensor &Tensor::set_vec_to_tensor(const vector<float> &vec)  {
        to_cpu(false);
        std::memcpy(cpu<float>(),vec.data(),vec.size()*sizeof(float) );
        return *this;
    }
    ...
    Tensor& Tensor::set_norm_mat_gray(int n, const cv::Mat& image) {
        Assert(image.channels() == 1 && !image.empty());
        Assert(ndims() == 4 && n < shape_[0]);
        to_cpu(false);
        ...
        cv::Mat ms = cv::Mat(height, width, CV_32F, cpu<float>(n));
        inputframe.copyTo(ms);

//        split(inputframe, ms);
        Assert((void*)ms.data == (void*)cpu<float>(n));
//        ms = (ms * scale ) ;
        return *this;
    }
