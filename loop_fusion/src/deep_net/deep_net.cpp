...
//            sp_in_image->set_norm_mat_gray(0,image_gray);
            ...
            //----------------------获取指针----------------------------
            auto*  des = sp_out_descriptors->cpu<float>();
            auto*  keypoints = sp_out_keypoints->cpu<int>();
            auto*  scores =  sp_out_scores->cpu<float>();
...
//            cout<<width_adj<<" "<<height_adj<<endl;
...
//            sp_re_out_scores->resize(scores_re_dim);
        ...
        void lg_matcher(std::vector<cv::Point2f> &lg_in_kpts0_,
                        ...
                        const int& height_1,const int& width_1) override {
...
//            float scale = 320;          //max(image.height,image.width)/2
            ...
            //如果是vins的480x752.
            float shift_width = width_adj/2;    // image.width/2
            float shift_height = height_adj/2;   //image.height/2
            float scale = max(shift_width,shift_height);          //max(image.height,image.width)/2
...
//            lg_in_kpts1 = lg_in_kpts1_norm->clone();
            ...
            lg_in_kpts0->resize({1,(int)lg_in_kpts0_.size(),2});
            lg_in_kpts1->resize({1,(int)lg_in_kpts1_.size(),2});
            lg_in_desc0->resize({1,(int)lg_in_kpts0_.size(),256});
            lg_in_desc1->resize({1,(int)lg_in_kpts1_.size(),256});
...
//            lg_in_desc1->copy_from_gpu(0,sp_out_descriptors->gpu(),lg_in_desc1->numel());
            ...
            float scale_width_0 = width_adj/width_0;
            float scale_height_0 = height_adj/height_0;

            float scale_width_1 = width_adj/width_1;
            float scale_height_1 = height_adj/height_1;
...
namespace MixVPR_net{
    ...
    class MixVPRImpl : public MixVPR{
    public:
        bool startup(const std::string &engine_path , int gpuid){
            gpu_ = gpuid;
            TRT::set_device(gpuid);

            mix_model_ = TRT::load_infer(engine_path);
            if(mix_model_ == nullptr){
                INFOE("Load model failed: %s", engine_path.c_str());
                return false;
            }
            mix_model_->print();
            stream_ = mix_model_->get_stream();

            return true;
        }

        void mix_extractor(const cv::Mat& image) override {
            ...
            if(image.channels() ==1)
            {
                cv::cvtColor(image_copy,image_copy,CV_GRAY2BGR,3);
            }

            mix_in_image = mix_model_->tensor("img");

            auto tensor = mix_in_image;
            ...
            affineMatrix.compute(image_copy.size(), input_size);

            tensor->resize(1, 3, 320, 320).to_gpu();

            size_t size_image = image_copy.cols * image_copy.rows * image_copy.channels();
            ...
            // speed up
            memcpy(image_host, image_copy.data, size_image);
            memcpy(affine_matrix_host, affineMatrix.d2i, sizeof(affineMatrix.d2i));
            checkCudaRuntime(cudaMemcpyAsync(image_device, image_host, size_image, cudaMemcpyHostToDevice, preprocess_stream));
            checkCudaRuntime(cudaMemcpyAsync(affine_matrix_device, affine_matrix_host, sizeof(affineMatrix.d2i), cudaMemcpyHostToDevice, preprocess_stream));
...
//            auto normalize_ = CUDAKernel::Norm::alpha_beta(1 / 255.f, 0.f, CUDAKernel::ChannelType::Invert);
            ...
            float mean[] = {0.406, 0.456, 0.485};
            float std[]= {0.225, 0.224, 0.229};
            auto normalize_ = CUDAKernel::Norm::mean_std(mean, std, 1 / 255.f, CUDAKernel::ChannelType::Invert);

            CUDAKernel::warp_affine_bilinear_and_normalize_plane_mix(
                    image_copy.channels(),image_device, image_copy.cols * image_copy.channels(), image_copy.cols, image_copy.rows,
                    tensor->gpu<float>(), 320, 320,
                    affine_matrix_device, 114,
                    normalize_, preprocess_stream
            );
            ...
            mix_model_->synchronize();
            mix_model_->forward();

            mix_out_descriptors = mix_model_->output(0);

            auto* des = mix_out_descriptors->cpu<float>();

            mix_des = vector<float>(des,des + 512);

            auto time2 = std::chrono::high_resolution_clock::now();
            cout<<"mix time = "<<std::chrono::duration<float, std::milli>(time2 - time1).count()<<endl;

        }

        void sort_in_faiss(float* xq,float* db,int nb)
        {
            static int flag =0;
            int d = 512;      // dimension
            int nq = 1;  // nb of queries
...
//    faiss::IndexFlatL2 index(d); // call constructor
            ...
            index.add(nb, db); // add vectors to the index
            //开始真正的向量检索，nq表示待检索的向量数量，xq表示待检索的向量，k表示top k ,D  和 I 表示返回的距离和索引Index
            { // search xq
                idx_t* I = new idx_t[k * nq];
                float* D = new float[k * nq];
                ...
                for (int i = 0; i < nq; i++) {
                    for (int j = 0; j < k; j++)
                    {
                        ...
                        sim_map[flag].push_back(I[i * k + j]);
                    }
                    printf("\n");
                }
        ...
        void test_in_dataset(const std::string filepath) override{
            std::vector<cv::String > namearray;

            cv::glob(filepath,namearray);
            std::sort(namearray.begin(), namearray.end());

            for(auto &img:namearray)
            {
                auto image = cv::imread(img);
                mix_extractor(image);
                auto* xq = mix_des.data();

                if(descriptors_database.size()/512 > 15)
                {   vector<float> test;
                    test.resize((descriptors_database.size()/512 -15)*512);
                    test = vector<float>(descriptors_database.begin(),descriptors_database.begin()+(descriptors_database.size()/512 -15)*512);
                    sort_in_faiss(xq,test.data(), test.size()/512);
                }

                descriptors_database.insert(descriptors_database.end(),mix_des.begin(),mix_des.end());
            }

            int n = descriptors_database.size()/512;
            for(int i = 3; i <= n; i++)
            {

                auto img0 = cv::imread(namearray[i]);
                auto img1 = cv::imread(namearray[sim_map[i].at(0)]);
                auto img2 = cv::imread(namearray[sim_map[i].at(1)]);
                auto img3 = cv::imread(namearray[sim_map[i].at(2)]);
                putText(img0, "cur_frame:  " , cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,0,255),2);
                putText(img1, "top1_frame:  " +to_string(top_sim[3*i]), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,0,255),2);
                putText(img2, "top2_frame:  " +to_string(top_sim[3*i+1]), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,0,255),2);
                putText(img3, "top3_frame:  " +to_string(top_sim[3*i+2]), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0,0,255),2);


                cv::hconcat(img0,img1,img1);
                cv::hconcat(img1,img2,img2);
                cv::hconcat(img2,img3,img3);
                cv::imshow("666",img3);
                cv::waitKey(0);
            }
        }
    ...
    private:
        ...
        shared_ptr<TRT::Infer> mix_model_;
        //1x3x320x320
        shared_ptr<TRT::Tensor> mix_in_image;

        //dims = 512
        shared_ptr<TRT::Tensor> mix_out_descriptors;
    ...
    shared_ptr<MixVPR> creat_mix (const std::string & engine_path, const int& engine_type ,int gpuid){
        shared_ptr<MixVPRImpl> instance(new MixVPRImpl{});
        if(!instance->startup(engine_path,gpuid))
            instance.reset();
        return instance;
    }
