...
//    shared_ptr<Estimator_net::Estimator> deep_net_estimator;
    ...
    //mix_des
    void compute_mix_des();
    void sort_vec_faiss();

    void compute_mix_des_test();
    void sort_vec_faiss_test();


    //sp
    void computeWindowSuperpoint();
    void computeSuperpoint();


    //lg
    void light_glue_matcher(vector<cv::Point2f> &matched_2d_old, vector<cv::Point2f> &matched_2d_old_norm, vector<uchar> &status,
    ...
    std::vector<float> window_local_descriptors;
    vector<cv::KeyPoint> sp_keypoints;
