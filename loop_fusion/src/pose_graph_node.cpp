...
void image_process() {
    while (!image_buf.empty()) {
        image_callback(image_buf.front());
        image_buf.pop();
    }
}
...
int main(int argc, char **argv)
{
    ...
    fsSettings["USE_SP"] >> USE_SP;
    fsSettings["loop_top_thres"] >> loop_top_thres;
    fsSettings["loop_back_thres"] >> loop_back_thres;
    fsSettings["MAX_THETA_DIFF"] >> MAX_THETA_DIFF;
    fsSettings["MAX_POSE_DIFF"] >> MAX_POSE_DIFF;
    fsSettings["height_adj"] >> height_adj;
    fsSettings["width_adj"] >> width_adj;
    fsSettings["F_BAG_PATH"] >> BAG_PATH;
//    printf("height_adj :%d width_adj:%d\n", height_adj,width_adj);
...
//#define FAST_BAG
    ...
    for (rosbag::View::iterator it = view.begin(); it != view.end(); ++it) {
        auto data_tmp = *it;
        if (data_tmp.getTopic() == IMAGE_TOPIC) {
            image_buf.emplace(data_tmp.instantiate<sensor_msgs::Image>());
        }
    }
    ROS_INFO("time start = %.6f", time_st.toSec());
    ROS_INFO("time end   = %.6f", time_end.toSec());
    ROS_INFO("Image0_Size = %lu", image_buf.size());
    usleep(5000000);
#else
#endif
